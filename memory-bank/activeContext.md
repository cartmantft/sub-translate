...
### 🎯 현재 우선순위: Issue #23 - [UI/UX] 상세 페이지 자막 편집기 기능 고도화 및 레이아웃 개선
- **목표**: 기존의 단순 자막 뷰어를 기능이 풍부한 인라인 편집기로 전환
- **문제**: 
  - 현재 자막 뷰어는 읽기 전용으로 편집 기능 없음
  - 비디오와 편집기 영역 크기가 고정되어 사용자 맞춤 불가
  - 3-탭 뷰가 직관적이지 않고 여러 번 클릭 필요
  - 자막 세그먼트의 시각적 컨텍스트(썸네일) 부재
- **해결 방향**:
  - Allotment 라이브러리로 리사이즈 가능한 분할 패널 구현
  - VideoThumbnail 컴포넌트로 각 자막에 비디오 썸네일 표시
  - 세그먼트 컨트롤 UI로 번역/원본/모두 보기 옵션 제공
  - 인라인 편집 기능으로 자막 텍스트 즉시 수정 가능
  - 변경사항 저장 버튼으로 서버 업데이트
- **GitHub 이슈**: https://github.com/cartmantft/sub-translate/issues/23
- **구현 단계**: 계획 수립 완료, **feature/issue-23-subtitle-editor-enhancement 브랜치에서 개발 진행 중**
