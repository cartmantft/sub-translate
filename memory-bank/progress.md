...
## 🔄 현재 진행 중: Issue #7 - 비디오-자막 동기화 및 사용성 개선 (2025-07-02)

### 목표
- **비디오 재생 시 현재 자막 하이라이트**: 사용자가 어떤 자막이 현재 재생되고 있는지 쉽게 파악
- **향상된 비디오 진행바**: 더 잘 보이는 진행바로 비디오 진행 상황 명확히 표시
- **최적화된 탭 순서**: 한국어 사용자 우선으로 "번역" → "원본" → "원본+번역" 순서
- **기본 탭 설정**: 가장 중요한 "번역" 탭을 기본 선택으로 설정

### 기술적 구현 계획
- **VideoPlayer.tsx**: onTimeUpdate 콜백 추가 + 진행바 스타일 개선
- **UnifiedSubtitleViewer.tsx**: currentTime prop 추가 + 하이라이트 로직 + 탭 재배치
- **MainContent.tsx**: 비디오-자막 시간 동기화 연결
- **테스트**: Playwright E2E 테스트로 모든 기능 검증

### 예상 효과
- 비디오와 자막 간 완벽한 시각적 동기화
- 한국어 사용자 중심의 직관적인 인터페이스
- 기존 기능 (자막 클릭으로 비디오 점프) 유지
- 반응형 레이아웃과의 완벽한 호환성
