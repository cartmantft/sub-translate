          ...
          <div>
            <h2 className="text-2xl font-semibold text-gray-800 mb-4">
              ...
              {parsedSubtitles.length > 0 && (
                <span className="text-sm text-gray-500 ml-2">(클릭하면 해당 시간으로 이동)</span>
              )}
            </h2>
            {parsedSubtitles.length > 0 ? (
              <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200 max-h-96 overflow-y-auto">
                <div className="space-y-3">
                  {parsedSubtitles.map((subtitle, index) => (
                    <div 
                      key={subtitle.id} 
                      className="border-b border-gray-200 pb-3 last:border-b-0 cursor-pointer hover:bg-gray-50 p-2 rounded transition-colors"
